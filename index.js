    ...
    try {
        ...
        client.on('new-msg', (message) => {
            ...
            const sender = isGroup ? message.participant : from
                        //tes hehe
                        if (body.toLowerCase() === 'bot') Client.reply(from, 'Iya, ada yang bisa aku bantu?', message) 
                        if (body.toLowerCase() === 'tes') Client.reply(from, 'Masuk Beb :v', message) 
			if (global.tebakgambar[from] && global.tebakgambar[from].id && global.tebakgambar[from].jawaban.toLowerCase() == body.toLowerCase()) Client.reply(from, `Tebak Gambar Berhasil Dijawab! Oleh @${sender.split("@")[0]}`, message).then(() => global.tebakgambar[from] = {}) 
